...
export default function Restaurants(props) {
  ...
  //Estado donde guardaremos el listado de restaurantes
  const [restaurants, setRestaurants] = useState([]);
  //donde comenzara el scroll infinito
  const [startRestaurants, setStartRestaurants] = useState(null);
  const [isLoading, setIsLoading] = useState(false);
  //total de restaurantes que tiene nuestra aplicacion
  ...
  //Conectarnos a la base de datos y traernos todos los restaurantes
  useEffect(() => {
    //Aqui obtenemos el total de restaurantes
  ...
  const handleLoadMore = async () => {
    const resultRestaurants = [];
    resultRestaurants.length < totalRestaurants && setIsLoading(true);
  ...
  return (
    <View style={styles.viewBody}>
      ...
      {user && (
        //Este componente esta mas abajo
        <AddRestaurantButton
          navigation={navigation}
